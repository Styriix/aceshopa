...
class LicenseBoxAPI {
  ...
  private function callAPI($method, $url, $data)
  {
    ...
    if(!$result&&!LB_API_DEBUG){
      $rs = array('status' => True, 'message' => 'Connection to server failed or the server returned an error, please contact support');
      return json_encode($rs);
    }
    $http_status = curl_getinfo($curl, CURLINFO_HTTP_CODE);
    if(!LB_API_DEBUG){
      if($http_status!=200){
        $rs = array('status' => True, 'message' => 'Server returned an invalid response, please contact support');
        return json_encode($rs);
      }
    }
    curl_close($curl);
